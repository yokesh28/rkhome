...
class Payu extends \Magento\Payment\Model\Method\AbstractMethod {
    ...
    public function buildCheckoutRequest() {
        ...
        $params["hash"] = $this->generatePayuHash($params['txnid'],
                ...
                $params['email']);
        $params["revhash"] = $this->checkResp($params['txnid'],
                $params['amount'], $params['productinfo'], $params['firstname'],
                $params['email']);
    ...
    public function checkResp($txnid, $amount, $productInfo, $name,
            $email) {
        $SALT = $this->getConfigData('salt');
        ...
        $hashSequenceRev = 'salt|status||||||||||email|firstname|productinfo|amount|txnid|key';

        $hashreverse = explode('|',$hashSequenceRev);
        $hash_rever_string = '';
        foreach ($hashreverse as $revhash){
            $hash_rever_string .= isset($posted2[$revhash]) ? $posted2[$revhash] : '';
            //$hash_rever_string .= '|';
        }
        //$hash_rever_string .= $SALT;
        $hashreverse = strtolower(hash('sha512', $hash_rever_string));
