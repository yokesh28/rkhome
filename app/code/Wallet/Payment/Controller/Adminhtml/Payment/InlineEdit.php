        ...
        if ($this->getRequest()->getParam('isAjax')) {
            ...
            if (!count($postItems)) {
                ...
                foreach (array_keys($postItems) as $modelid) {
                    /** @var \Wallet\Payment\Model\Payment $model */
                    $model = $this->_objectManager->create('Wallet\Payment\Model\Payment')->load($modelid);
                    try {
                        $model->setData(array_merge($model->getData(), $postItems[$modelid]));
                        $model->save();
                    } catch (\Exception $e) {
                        $messages[] = "[Payment ID: {$modelid}]  {$e->getMessage()}";
                        $error = true;
                    }
                }
            }
